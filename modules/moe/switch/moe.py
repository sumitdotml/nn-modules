...
class RoutingInfo(NamedTuple):
    ...
    logits: Tensor  # [total_tokens, n_experts]


class MoE(nn.Module):
    """
    MoE variant for Switch-style auxiliary load balancing (Switch Transformer / Mixtral family).

    This variant caches:
    - `indices` (selected experts) and
    - router `logits` (dense scores) via the router.
    """
    ...
    @property
    def last_router_logits(self) -> Optional[Tensor]:
        return self.router.last_logits

    @property
    def last_routing_info(self) -> Optional[RoutingInfo]:
        indices = self._last_routing_indices
        logits = self.router.last_logits
        if indices is None or logits is None:
            return None
        return RoutingInfo(indices=indices, logits=logits)
